...
"""
class LandmarksDataset(Dataset):
    def __init__(self, data_dir, image_filenames, transform=None):
        self.images_dir = os.path.join(data_dir, 'Images')
        self.landmarks_dir = os.path.join(data_dir, 'landmarks')
        self.image_files = image_filenames 
        self.landmarks_files = [os.path.splitext(f)[0] + '.npy' for f in self.image_files]
    ...
    def __getitem__(self, idx):
        ...
        landmarks_dict = extract_landmarks(img_basename, self.landmarks_dir, split=False)
        valid_landmarks = [v for v in landmarks_dict.values() if isinstance(v, np.ndarray) and v.ndim == 2]
        landmarks = np.concatenate(valid_landmarks, axis=0) if valid_landmarks else np.empty((0, 2))
